...
DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
	...
	// Arxikopoihseis metavlhtwn. O logos pou tis arxikopoihsame mesa sto struct kai oxi px. san ksexwristes global metavlites einai oti theloume na diaxeiritsoume
	// tis leitourgies ths vashs. Etsi eykola me aples metavlites tha markaroume tis krisimes perioxes pou theloume.
	//Etsi, parathroume oti h metavliti self kanei calloc me to megethos ths vashs ARA sigoura xreiazetai sthn desmeysh xwrou pou tha diaxeiristoume emeis.
	///////////////////////////////////////////////
...
int db_add(DB* self, Variant* key, Variant* value)
{
	...
	while(self->rc > 0){	// Oso yparxoun akoma readers tote
		pthread_cond_wait(&self->checkread, &self->orderhandler); // Blockarei to reading. Edw theloume na kanoume put(write) sth vash.
		//To dwsmeno mutex mporei na epikoinwnhsei kai me to read kai write opote blockarontas to read mas anakatheythinei sto write.
	}
	self->ifw = 1; //Efoson tha ginei write, kanoyme th metavliti 1.
	...
	haswritten = memtable_add(self->memtable, key, value); // Edw ginetai to "write" sti vasi.Thelw na to vriskw MESA sthn krisimi perioxi, gia ayto to vazw se metavliti kai to epistrefw EKTOS perioxis.
	self->ifw = 0; //Egine ena write, ksana sto 0 h metavliti.
	pthread_cond_signal(&self->checkwrite); // Kseblokarei to write. Tha to kanei mono an entopistei blockarismeno apo thn db_get parakatw.
...
int db_get(DB* self, Variant* key, Variant* value)
{
	...
	while(self->ifw == 1){
		pthread_cond_wait(&self->checkwrite, &self->orderhandler);// Blockarei to writing. Edw theloume na kanoume get(read) apo th vash.
	}
	self->rc = self->rc + 1; //Efoson theloume read, vazei allon enan reader.
	
    if (memtable_get(self->memtable->list, key, value) == 1){ //Edw kanoume thn prosthiki ths dikh mas metavlitis pou tha krataei to apotelesma tou read MESA sthn krisimi perioxi.
        hasread = 1; //edw tha parameinei apla 1 kai telos.
	}else{
		hasread = sst_get(self->sst, key, value); //Edw tha epistrepsei thn arxiki timi pou ekane return o default kwdikas.
	}

	self->rc = self->rc - 1; //Egine ena read, ara meiwnw antistoixa kata 1.
	
	if (self->rc == 0){
		pthread_cond_signal(&self->checkread); //Teleiwsan oi anagnwstes. Kane "SIGNAL" kai ksekleidwse thn read.
	}
